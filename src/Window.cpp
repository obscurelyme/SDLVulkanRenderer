...
SDL_Window* CoffeeMaker::Window::CreateWindow(std::string name, int width, int height) {
  DestroyWindow();
  gWindow =
      SDL_CreateWindow(name.c_str(), SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height,
                       SDL_WINDOW_VULKAN | SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE | SDL_WINDOW_FULLSCREEN_DESKTOP);
  return gWindow;
}
